...
Она называется **"Reset Configuration"** и находится в правом верхнем углу.

**Все что нужно знать для старта:**

В кластере Tarantool есть две служебные роли: Router и Storage. 
...
- пользователей
- видео с их описаниями и предварительно подсчитанным кол-вом лайков
- **сами лайки**
...
В Tarantool есть два встроенных движка хранения: memtx и vinyl. Memtx хранит все данные в оперативной памяти, при этом асинхронно записывая на диск, чтобы ничего не потерялось.

Движок Vinyl — это классический движок для хранения данных на жестком диске. Он оптимизирован для большого количества операций записи данных. 

В данной задаче у нас будет большое количество и чтений и записей. Поэтому используем memtx.

Мы создали три спейса (таблицы) в memtx и для каждого из спейсов создали необходимые индексы.
...
**Важно:** название `bucket_id` зарезервировано. Если вы выберете другое название, то шардирование для этого спейса работать не будет. Если в проекте шардирование не используется, то его можно убрать.

Чтобы понять, по какому полю шардировать данные, Tarantool использует `sharding_key`. Он указывает на поле в спейсе, по которому будут шардироваться записи.
Tarantool возьмет хеш от этого поля при вставке, вычислит номер бакета и подберет для записи нужный Storage. 

Бакеты могут повторяться, а каждый Storage хранит определенный диапазон бакетов.
...
Для простоты в методе `like_video` операции `update` и `insert_object` находятся не в транзакции. Для изучения работы транзакций, читайте [документацию Tarantool в разделе "Транзакции"](https://www.tarantool.io/en/doc/latest/book/box/atomic/). 
...
Так как у нас пока поднят всего один Storage и один Router, данные хранятся только на одном узле. 
...
Щелкаем на нужные роли и создаем шард (репликасет).
...
Чтобы задействовать новый шард, надо выставить его вес в единицу. Заходим снова на вкладку "Cluster", переходим в настройки `s2-master`, выставляем Replica set weight в 1 и применяем.
...
Выставляем в настройках `s1-master` Replica set weight в 0 и применяем. Подождем пару секунд, заходим в Space Explorer и смотрим на данные в `s2-master`: все данные автоматически мигрировали на оставшийся шард. 
...
Используйте Docker контейнер с СentOS 8 или механизм WSL и следуйте инструкции по установке под Linux.
